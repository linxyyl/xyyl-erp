...
class Service
{
    ...
    public static function submitOrder($amount, $orderid = null, $type = null, $title = null, $notifyurl = null, $returnurl = null, $method = null)
    {
        ...
        if ($type == 'alipay') {
            ...
            switch ($method) {
                case 'web':
                    //电脑支付,跳转
                    $pay->web($params)->send();exit;
                    break;
                case 'wap':
                    //手机网页支付,跳转
                    $pay->wap($params)->send();exit;
                    break;
                case 'app':
                    //APP支付,直接返回字符串
                    $pay->app($params)->send();exit;
                    break;
                case 'scan':
                    //扫码支付,直接返回字符串
                    $html = $pay->scan($params)->toArray();
                    break;
                case 'pos':
                    //刷卡支付,直接返回字符串
                    //刷卡支付必须要有auth_code
                    $params['auth_code'] = $auth_code;
                    $html = $pay->pos($params)->toArray();
                    break;
                default:
            ...
            switch ($method) {
                case 'web':
                    ...
                    $_params['sign'] = md5(implode('', $params) . $config['app_id']);
                    $endpoint = addon_url("epay/api/wechat");
                    $_html = self::buildPayHtml($endpoint,$_params);
                    \response($_html)->send();
                    exit();
                    break;
                case 'mp':
                ...
                case 'miniapp':
                    ...
                    $html = $pay->miniapp($params)->toArray();
                    break;
                default:
            }
        }
    ...
    protected static function buildPayHtml($endpoint, $params)
    {
    ...
    public static function getConfig($type = 'wechat')
    {
        ...
        if ($config['log']) {
            $config['log'] = [
                'file'  => runtime_path() . '/epaylogs/' . $type . '-' . date("Y-m-d") . '.log',
                'level' => 'debug'
            ];
        }
        ...
        if (isset($config['ali_public_key']) && substr($config['ali_public_key'], 0, 6) == '/epay/') {
            $config['ali_public_key'] = ADDON_PATH . $config['ali_public_key'];
        }
        if (isset($config['app_cert_public_key']) && substr($config['app_cert_public_key'], 0, 6) == '/epay/') {
            $config['app_cert_public_key'] = ADDON_PATH . $config['app_cert_public_key'];
        }
        if (isset($config['alipay_root_cert']) && substr($config['alipay_root_cert'], 0, 6) == '/epay/') {
            $config['alipay_root_cert'] = ADDON_PATH . $config['alipay_root_cert'];
        }
