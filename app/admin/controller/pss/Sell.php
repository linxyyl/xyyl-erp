...
class Sell extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\pss\Sell;
        $this->info['companyList'] = \app\admin\model\pss\Company::getOneList();
        $this->info['stockList'] = Stock::getOneList();
        $this->info['goodsList'] = Goods::getOneList();
        $this->view->assign("companyList",  $this->info['companyList']);
        $this->view->assign("stockList",  $this->info['stockList']);
        $this->view->assign("goodsList",  $this->info['goodsList']);
        $this->assignconfig('companyList', $this->info['companyList']);
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
                $row->visible([
                    'id', 'admin_id', 'admin_name', 'company_id', 'stock_id', 'stock_goods_id', 'goods_id', 'goods_number',
                    'goods_price', 'other_price', 'transport_price', 'amount_price', 'status', 'createtime',
                    'updatetime', 'deletetime', 'remark', 'code'
                ]);
            }
    ...
    public function confirmPut()
    {
        ...
        if ($model->goods_number > $goods->number) {
            $this->error('销售仓库商品数量不足!');
        }
