...
/**
 * ylyf 随机token生成
 * @param int $length 最大256
 * @return string
 */
function ylyf_token(int $length = 64): string
{
    if ($length > 256) {
        $length = 256;
    }
 ...
 * @return string
 */
function ylyf_password(string $password): string
{
    $password = md5(crypt($password, md5($password)));
...
/**
 * ylfy密码验证
 * @param string $user_password 用户明文密码
 * @param string $db_password 数据库密文
 * @return bool 正确返回 true 错误返回 false
 */
function ylyf_verify_password(string $user_password, string $db_password): bool
{
    $u_p = md5(crypt($user_password, md5($user_password)));

    return password_verify($u_p, $db_password);
...
function getBrowser(){
    ...
    if(strpos($agent,'MSIE')!==false || strpos($agent,'rv:11.0'))
        ...
        return "IE <span style='color: #ff0000'> (为了您的正常使用,推荐使用谷歌浏览器) </span>";
